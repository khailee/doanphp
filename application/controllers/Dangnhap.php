...
class Dangnhap extends CI_Controller {
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_user']    = 'wangwei.design96@gmail.com';
            $config['smtp_pass']    = 'xhfsqizxsvfgeurs';
            ...
            $this->email->from('wangwei.design96@gmail.com', 'Smart Store');
            $this->email->to($email);
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
               $password_new = ($_POST['re_password']);
